...
Описание **одного объекта** в файле `.jsonl` (одна строка после `json.loads`): выход **`convert.py`**. Типичный файл по умолчанию — **компактный**: без ключей с **`null`** и без пустых **`[]`**/**`{}`** там, где значение пустое после нормализации (см. **`--include-null-keys`** в README). Схема допускает и «полный» объект с **`null`** для проверки выборок с **`--include-null-keys`**.
...
- У элементов **`Decisions[]`** поле **`Id`** может быть `null` или **отсутствовать** как ключ при компактном JSON. JSON Schema это **разрешает** (`string | null` и отсутствие ключа не проверяется как отдельное правило); отдельное правило «в реляционный импорт такая позиция не попадает в таблицу `decisions`» см. в [`sql_convert.md`](sql_convert.md).
- Поля **`INN`**, **`KPP`**, **`OGRN`**, **`EduOrgINN`**, **`EduOrgKPP`**, **`EduOrgOGRN`** и т.д. в схеме — **`string | null`**; в типичном выходе **`convert.py`** некорректные идентификаторы из XML **не** сохраняются как строка с буквами: ключ **отсутствует** или значение **`null`** (см. README и **`convert.cast_id_number`**).
