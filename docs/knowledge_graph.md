...
- По умолчанию `convert.py` **не** включает в JSONL сертификаты с корневым **`StatusName`** из **«Недействующее»**, **«Прекращено»**, **«Лишен аккредитации»** и **удаляет** из **`Supplements[]`** элементы с теми же статусами в поле приложения; **не** включает строки с псевдорегионом «за пределами РФ» на корневом `RegionName`; строки без валидного `EduOrgOGRN` по умолчанию **включаются**. Полный снимок как в XML по статусам — **`--include-inactive`**; по региону — **`--include-outside-rf-region`**; дополнительный срез — **`--omit-invalid-eduorg-ogrn`**; все ключи в JSON (в т.ч. `null`) — **`--include-null-keys`** (см. [`README`](../README.md)).
