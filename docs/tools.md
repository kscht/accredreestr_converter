...
- с дозаполнением пустых **`INN`** / **`OGRN`** в **`ActualEducationOrganization`** (корень и **`Supplements[]`**) при совпадении реестрового UID с согласованными донорами (**`fill_aeo_coherent_inn_ogrn`**, отключение: **`--no-fill-aeo-coherent-inn-ogrn`**).
...
После смены логики нормализации идентификаторов **или** дозаполнения AEO в **`convert.py`** пересоберите JSONL и заново запустите аудиты (**`audit_dataset_identity_fields.py`**, при необходимости регион/статус/AEO), чтобы **`examples/dataset_*_audit.json`** и счётчики **`nonempty_not_digits_only_after_clean`** / **`*borrowable*`** отражали текущий снимок.
