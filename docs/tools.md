...
После смены логики нормализации идентификаторов **или** дозаполнения AEO в **`convert.py`** пересоберите JSONL и заново запустите аудиты (**`audit_dataset_identity_fields.py`**, при необходимости регион/статус/AEO), чтобы **`examples/dataset_*_audit.json`** и счётчики **`nonempty_not_digits_only_after_clean`** / **`*borrowable*`** отражали текущий снимок. Выборку «филиал / пустой INN в supplement» пересобирайте **`extract_branch_supplement_aeo_inn_gap_jsonl.py`**, затем **`audit_branch_supplement_aeo_inn_gap.py`** (см. **`.gitignore`** для крупного **`examples/branch_supplement_aeo_inn_gap.jsonl`**).
