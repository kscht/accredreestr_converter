...
| `specs/edu_level_names_fz273_map.json` | Маппинг непустых **`EduLevelName`** к **`canonical_edu_level_names_fz273`**: в **`entries`** только отличия источник→цель и **`null`**-цели; совпадение с именем из канона **без** своей строки в **`entries`** — неявный **identity**. Ручные свёртки (напр. «Общее образование»→ООО). Покрытие vocab — **`tests/test_edu_level_names_fz273_map.py`**. Примеры: **`examples/certificate_lines_edu_level_name_obschee_obrazovanie_sample.jsonl`**, **`…_professionalnoe_obuchenie_sample.jsonl`**, **`…_professionalnoe_obrazovanie_sample.jsonl`**. В **`convert.py`** по умолчанию нормализует **`EducationalPrograms[].EduLevelName`**; отключение — **`--no-normalize-edu-level-names-fz273`**. |
...
| `tools/audit_dataset_edu_program_levels.py` | **`EduLevelName`** в **`Supplements[].EducationalPrograms[]`**: гистограмма, классификация (школа / нешкола / смесь; только по **непустым** уровням), учёт **пустых** `EduLevelName` (число программ и **число сертификатов** + разрез по классу); **`-p`**, **`--school-jsonl`**, **`--mixed-jsonl`**, **`--empty-level-jsonl`** [PATH] — полные строки выборок; **`--empty-program-jsonl`** [PATH] — по одной строке на программу с пустым уровнем (`certificate_id`, индексы, `program`); без PATH после флага — **`examples/edu_programs_empty_EduLevelName.jsonl`**; **не** указывайте тот же путь, что и входной JSONL: файл открывается на запись и перезапишет вход; крупные выборки полных сертификатов в **`.gitignore`**; по умолчанию `examples/dataset_edu_program_levels_audit.json` |
