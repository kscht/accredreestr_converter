...
| `specs/edu_level_names_fz273_map.json` | Маппинг непустых **`EduLevelName`** к **`canonical_edu_level_names_fz273`**: в **`entries`** только отличия источник→цель и **`null`**-цели; совпадение с именем из канона **без** своей строки в **`entries`** — неявный **identity**. Ручные свёртки: **«Общее образование»** → ООО, **«Профессиональное обучение»** → СПО (УГС 40.00.00), **«Профессиональное образование»** → СПО (карточка РСО-Алания); **`Не определен`** — без цели. Покрытие vocab — **`tests/test_edu_level_names_fz273_map.py`**. Примеры: **`examples/certificate_lines_edu_level_name_obschee_obrazovanie_sample.jsonl`**, **`…_professionalnoe_obuchenie_sample.jsonl`**, **`…_professionalnoe_obrazovanie_sample.jsonl`**. В **`convert.py`** не подключается. |
