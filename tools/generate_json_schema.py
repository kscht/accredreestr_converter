...
def build_schema_dict() -> dict[str, Any]:
    ...
    decision_id_schema["description"] = (
        "Идентификатор распорядительного документа в выгрузке. null или отсутствие ключа при "
        "компактном JSON (omit_null_keys по умолчанию), если в XML пустой тег — объект решения "
        "может содержать другие поля; реляционный импорт не добавляет строку в таблицу decisions "
        "без непустого Id, сертификат не отбрасывается."
    )
    ...
    return {
        ...
        "description": (
            ...
            "По умолчанию компактный JSON (без ключей с null и пустых коллекций после нормализации). "
            "ИНН/КПП/ОГРН в типичном выходе — только цифры после очистки или ключ отсутствует. "
