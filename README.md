...
#    По умолчанию: без «Недействующее» на корне; без псевдорегиона «за пределами РФ»; компактный JSON (без null и пустых []/{});
...
| `convert.py` | Конвертация XML → JSONL, CLI; дополнение INN/OGRN в AEO при совпадении UID (отключение: **`--no-fill-aeo-coherent-inn-ogrn`**) |
...
| **Полный снимок по статусу** (в т.ч. «Недействующее» на корне, как в XML) | `python convert.py data/file.xml -o out/full.jsonl --include-inactive` |
...
| **Без дополнения INN/OGRN в AEO** при совпадении UID (строго как в сыром XML после нормализации скаляров) | `python convert.py data/file.xml -o out/raw_aeo.jsonl --no-fill-aeo-coherent-inn-ogrn` |

По умолчанию в JSONL **не** попадают сертификаты со **`StatusName` «Недействующее»** на корне (в отчёте **`omitted_inactive`** > 0, если такие строки были в XML). Чтобы включить их, задайте **`--include-inactive`**. По умолчанию **не** попадают строки с корневым **`RegionName` «образовательные учреждения, находящиеся за пределами Российской Федерации»** (**`omitted_outside_rf_region`** в отчёте); полный снимок по региону — **`--include-outside-rf-region`**. Строки **без** валидного **`EduOrgOGRN`** по умолчанию **включаются**; для среза — **`--omit-invalid-eduorg-ogrn`** (**`omitted_invalid_eduorg_ogrn`**). По умолчанию в каждой строке **не пишутся** ключи с **`null`**, пустой строкой и пустыми вложенными **`{}`**/**`[]`**; «как в сыром дереве парсера» — **`--include-null-keys`**. Флаги **`--omit-inactive`**, **`--omit-outside-rf-region`**, **`--omit-null-keys`** избыточны (то же, что умолчание). По умолчанию после парсера **дополняются** пустые **INN**/**OGRN** в **`ActualEducationOrganization`** (корень и приложения), если UID карточек совпадает: доноры — корневая AEO, **EduOrgINN**/**EduOrgOGRN** и supplement-карточки с тем же UID (см. `convert.fill_aeo_inn_ogrn_from_coherent_certificate_sources`); в JSON-отчёте — **`total.aeo_coherent_inn_ogrn_fills`**.
...
Полный снимок по статусу (в XML в т.ч. «Недействующее»), **без** среза по ИНН/ОГРН (в `convert.py` таких срезов нет; опционально только `--omit-invalid-eduorg-ogrn`):
