...
| `tools/audit_dataset_identity_fields.py` | Сводка по **EduOrgINN**, **EduOrgOGRN**, **INN**/**OGRN** в корневом и вложенных **ActualEducationOrganization** (приложения): непустые значения, «только цифры» после очистки, **`would_drop_if_require_*`** для оценки потерь строк/карточек при выборе PK → `examples/dataset_identity_fields_audit.json` |
